<?php
	...
	$testdata = mysql_query($testquery,$con)or die("41：你还没选择课程~先选择一门课程开始学习吧~");
	$testrow = mysql_fetch_array($testdata);


//原始字符串
	$string_wrong = $testrow['wrong_q_id'];

	if(strlen($string_wrong)>1){
		$string_wrong = substr($string_wrong,0,strlen($string_wrong)-1);
	}
	

	$dataWrongSet = array();
	$wrongArray = array();
	$wrongArray = preg_split("/,/",$string_wrong,-1);

	if($test< 100 )$test = 100;

	for($intNumQuestion = 0 ; $intNumQuestion<count($wrongArray);$intNumQuestion++){
		// echo $wrongArray[$intNumQuestion];
		// echo '</br>';
		$queryForWrong = "SELECT title,answer1,answer2,answer3,answer4,id,answer,score from tbl_question ".
			  "where id= ".$wrongArray[$intNumQuestion];
		// echo $queryForWrong;
		// echo '</br>';
		$dataForWrongData = mysql_query($queryForWrong,$con)or die("wrong 47:$testquery");
		$dataForWrong = mysql_fetch_array($dataForWrongData);
		array_push($dataWrongSet,$dataForWrong);
	}

	$nNumWrongQ  = count($dataWrongSet);

	$nNumWrongQ  = count($dataWrongSet);
	
	// print_r($dataWrongSet);
	// print_r($wrongArray);
	...
	// if(($nNumRows=mysql_num_rows($dataWrongSet))<=0)
	// {
	// //echo "<p align=center>error1</p>";
	// //exit;
	// }
	//得到最大页码数MaxPage  
	$MaxPage=(int)ceil($nNumWrongQ/$gPageSize);
...
//dataWrongSet
if(($page-1)*$gPageSize<$nNumWrongQ)
// if(mysql_data_seek($data3,($page-1)*$gPageSize))
{
  ...
  if($arr)
  {
    ...
    echo "<form action='wrongQ.php' id='answerForm' method='POST'>";
    ...
    echo "<input name='ranswer' type='hidden' value='" . $arr[6] . "'/>";
     echo "<code>Wrong Qestion ID: $string_wrong</code>";
    echo "<label class='control-label'><h3>".$arr[5].":&nbsp".$arr[0]."</h3></label><br/>";
